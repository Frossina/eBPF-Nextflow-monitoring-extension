...
class Launcher {
    ...
    int run() {
         ...
         */
        //TODO: eBPF Start Marker
        long pid = ProcessHandle.current().pid()
        eBPF = new ProcessBuilder("/usr/bin/python3","monitor.py","-p",pid.toString())
        println(pid.toString())
        eBPF.inheritIO()
        procebpf = eBPF.start()
        long pyPid = procebpf.pid()
        println(pyPid.toString())
        while (true)
        {
            if (new File("ready").exists()) {
                new File("ready").delete()
                break
            }
            sleep(50)
        }
